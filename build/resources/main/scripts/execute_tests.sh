...
## check if the processjc file contains a working directory
if ! grep "workingdir" "$CONFIG_FILE" > /dev/null ; then
    echo "Missing 'workingdir' in '$CONFIG_FILE'"
    exit 1
else
    ## check if the working directory is specified in processjc
...
## check if the processjc file contains an install directory
