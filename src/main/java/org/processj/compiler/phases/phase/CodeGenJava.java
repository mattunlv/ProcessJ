    ...
    public final Void visitProtocolTypeDeclaration(final ProtocolTypeDeclaration protocolTypeDeclaration) {
        ...
        if (protocolTypeDeclaration.extend().size() > 0) {
            for (Name n : protocolTypeDeclaration.extend()) {
                ProtocolTypeDeclaration ptd = (ProtocolTypeDeclaration) topLvlDecls.get(n.toString());
...
//        String guard = (String) stat.visit(this);
    ...
    public final Void visitBinaryExpression(BinaryExpression binaryExpression)  {
        ...
        if ("==".equals(op) && (binaryExpression.getLeft() instanceof NameExpression && binaryExpression.getRight() instanceof NameExpression)
                && ((((NameExpression) binaryExpression.getLeft()).myDecl instanceof LocalDeclaration)
                && ((NameExpression) binaryExpression.getRight()).myDecl instanceof LocalDeclaration)) {
            LocalDeclaration ld1 = (LocalDeclaration) ((NameExpression) binaryExpression.getLeft()).myDecl;
            LocalDeclaration ld2 = (LocalDeclaration) ((NameExpression) binaryExpression.getRight()).myDecl;
    ...
    public final Void visitChannelReadExpression(ChannelReadExpression channelReadExpression) throws Phase.Error  {
        ...
        // end of the channel
        Expression chanExpr = channelReadExpression.getChannelExpression();
        // 'c' is the name of the channel
    ...
    public final Void visitInvocationExpression(InvocationExpression invocationExpression) throws Phase.Error  {
        ...
        if(invocationExpression.ignore) {
            ...
            Log.log(invocationExpression, "Visiting a " + invocationExpression);
        ...
        if(invocationExpression.getProcedureName().equals("sync") || invocationExpression.getProcedureName().equals("fork")) {
            Log.log(invocationExpression, "Visiting Invocation (" + invocationExpression + ")");
            // TODO:
        }
    ...
    public final Void visitArrayLiteralExpression(ArrayLiteralExpression arrayLiteralExpression) throws Phase.Error  {
        ...
        // Visit the Type
        arrayLiteralExpression.getType().visit(this);

        // Visit the Expressions
        arrayLiteralExpression.getExpressions().visit(this);
    ...
    public final Void visitProtocolLiteralExpression(final ProtocolLiteralExpression protocolLiteralExpression) throws Phase.Error {
        ...
        // Visit the Expressions
        protocolLiteralExpression.getExpressions().visit(this);
    ...
    public final Void visitRecordLiteralExpression(final RecordLiteralExpression recordLiteralExpression) throws Phase.Error  {
        ...
        // Visit the Expressions
        recordLiteralExpression.getRecordMemberLiterals().visit(this);
    ...
    public final Void visitRecordMemberLiteral(final RecordMemberLiteralExpression recordMemberLiteralExpression) throws Phase.Error {
        ...
        recordMemberLiteralExpression.getExpression().visit(this);
    ...
    public final Void visitAltStatement(AltStatement altStatement)  {
        ...
        if (altStatement.isReplicated()) {
            Log.log(altStatement, "Visiting a Dynamic or Replicated Alt");
...
//                tlocals.add(stTimeout.render());
        ...
        try {
            new LocalDeclaration(new PrimitiveType(PrimitiveType.IntKind), n, null, false /* not constant */).visit(this);
        } catch (Phase.Error error) {
            throw new RuntimeException(error);
        }
        // Create a tag for this local alt declaration
    ...
    private ProcedureTypeDeclaration createAnonymousProcTypeDecl(Statement st)  {
        return new ProcedureTypeDeclaration(new Sequence(), // Modifiers
                null, // Return type
    ...
    protected static void GenerateNativeLibraryCode(final Phase phase, final SymbolMap scope) throws Phase.Error {
        ...
        if(scope.definesLibraryPragma()) {
            ...
            final String packageName    = scope.getPackageName();
    ...
    public static class IOCallsRewrite extends Phase {
        ...
        public final Void visitInvocationExpression(InvocationExpression invocationExpression) {
            Log.log(invocationExpression, "Attempting to rewrite invocation of " + invocationExpression);

            if (invocationExpression.targetProc != null) {

                Log.log(invocationExpression,"targetProc is " + invocationExpression.targetProc);

                if (invocationExpression.targetProc.getPackageName().equals("io") &&
                    ...
                    return null;//invocationExpression;
                }
            }

            if (invocationExpression.getParameters() != null) {
                if(invocationExpression.getParameters().size() == 0) {
                    Log.log(invocationExpression, "Params are empty");
                    return null; //invocationExpression;
                }

                boolean rewritten = false;
                Sequence<Expression> params = invocationExpression.getParameters();
                Sequence<Expression> newParams = new Sequence<Expression>();
                Log.log(invocationExpression, "Invocation of " + invocationExpression + " has argument(s):");
                for (int i = 0; i < params.size(); ++i) {
                    if (params.child(i) instanceof BinaryExpression) {
                        Log.log(invocationExpression, params.child(i).toString());
                        if (checkForString((BinaryExpression)params.child(i)) == true) {
                            rewritten = true;
                            Log.log(invocationExpression, "string concatenation found, rewriting.");
                            newParams.merge(rebuildParams(extractParams((BinaryExpression)params.child(i))));
                        }
                    }
                }

                Log.log(invocationExpression, "Received params from extractParams():");
                for (int i = 0; i < newParams.size(); ++i) {
                    Log.log(invocationExpression, newParams.child(i).toString());
                }
        ...
        public Sequence<Expression> extractParams(BinaryExpression be) {
            ...
            Log.log(be, "Left is " + be.getLeft().toString() + ", right is " + be.getRight().toString());

            Sequence<Expression> newParams = new Sequence<Expression>();

            if (be.getLeft() instanceof BinaryExpression) {
                newParams.merge(extractParams((BinaryExpression)be.getLeft()));
            } else if (be.getLeft() instanceof PrimitiveLiteralExpression || be.getLeft() instanceof ArrayAccessExpression) {
                newParams.append(be.getLeft());
            }

            newParams.append(be.getRight());
        ...
        public boolean checkForString(BinaryExpression be) {
            ...
            if (be.getLeft() instanceof PrimitiveLiteralExpression) {
                if(((PrimitiveLiteralExpression)be.getLeft()).getKind() == PrimitiveLiteralExpression.StringKind) {
                    return true;
                }
            }
            // Now the rhs
            if (be.getRight() instanceof PrimitiveLiteralExpression) {
                if(((PrimitiveLiteralExpression)be.getRight()).getKind() == PrimitiveLiteralExpression.StringKind) {
                    return true;
                }
            }
            ...
            if (be.getLeft() instanceof BinaryExpression) {
                left = checkForString((BinaryExpression)be.getLeft());
            }

            if(be.getRight() instanceof BinaryExpression) {
                right = checkForString((BinaryExpression)be.getRight());
            }
