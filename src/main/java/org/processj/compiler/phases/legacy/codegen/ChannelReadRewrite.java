...
public class ChannelReadRewrite {
    ...
    public AST go(AST a) {
        if (a instanceof AltCase) {
            ...
            for (int i = 0; i < as.getStatements().size(); i++)
                go(as.getStatements().child(i));
            return as;
            ...
            if (as.getRight().doesYield()) {
                // v = e, e yields
                Sequence seq2 = (Sequence) go(as.getRight()); // [ [ ... ] <e> ]
                ...
                // v = e, e does not org.processj.yield
                // returns [ [ ], v = e ]
            ...
            Sequence s = (Sequence) go(bl.getStatements()); // [<S1>; <S2>; ..., <Sn>]
            ...
            // TODO: this fails in org.processj.codegen for c.read().read() -- problem is in the type
            // checker
            // e.read()
            ...
            Sequence s = (Sequence) go(((ExprStat) a).getExpression());
            ...
            tempCounter--;
            Sequence s = (Sequence) go(is.getThenStatements());
            ...
            boolean yields = false; // once set to true a parameter that does not org.processj.yield must be lifted out
            ...
            for (int i = in.getParameters().size() - 1; i >= 0; i--) {
                ...
                if (yields || e.doesYield()) {
                    ...
                    if (e.doesYield()) { // e has a channel read in it
                        ...
                        s2.merge(s.child(0));
                        ret = s2.merge(ret);
                        in.getParameters().insert(i, new NameExpr(new Name(t)));
                        ...
                        s2.append(ld);
                        // t = e;
                        // No need to rewrite it as it doen't org.processj.yield.
            ...
            Sequence s = pb.getStatements();
            for (int i = 0; i < s.size(); i++) {
                Sequence res = (Sequence) go(s.child(i));
                s.insert(i, ((Sequence) res.child(0)).child(0));
            }
            final Sequence<Expression> barriers = new Sequence<>();
            pb.getBarrierSet().forEach(barriers::append);
            Sequence result = new Sequence(new Sequence(new ParBlock(s, barriers)));
            result.append(null);
            return result;
