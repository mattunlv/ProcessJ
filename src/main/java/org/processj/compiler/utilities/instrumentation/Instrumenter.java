package org.processj.compiler.utilities.instrumentation;
...
public class Instrumenter {
    ...
    public Instrumenter(String folder) {
        ...
        org.processj.compiler.Compiler.Info("in instrumenter!!");

        this.fullPath = folder;

        org.processj.compiler.Compiler.Info("================================");
        org.processj.compiler.Compiler.Info("*  Instrumenting classes in:   *");
        org.processj.compiler.Compiler.Info("================================");
        org.processj.compiler.Compiler.Info("Path:" + fullPath);
        org.processj.compiler.Compiler.Info("--------------------------------");
    ...
    public void execute() throws Exception {
        ...
        if (!directory.exists())
            org.processj.compiler.Compiler.Info("doesnt exist!!");
        ...
        if (directoryListing != null) {
            for (File file : directoryListing) {
                if (file.isFile() && isClassFile(file)) {
                    ...
                    org.processj.compiler.Compiler.Info("Instrumenting => " + file.getName());
        ...
        org.processj.compiler.Compiler.Info("Done!!");
    }
            ...
            if (insnNode.getType() == AbstractInsnNode.METHOD_INSN) {
                ...
                if (min.owner.equals(workingClassName)) {
                    ...
                    if (min.name.equals(MARK_YIELD)) {
                        //        	  org.processj.compiler.Compiler.Info("found org.processj.yield!!");
                        yields.add(min);
    ...
    private void insertReturnOnYields(final MethodNode mn,
        ...
        org.processj.compiler.Compiler.Info("ret_opcode=" + ret_opcode);
        ...
        if (jumpDest != null) {
            for (AbstractInsnNode node : yields) {
                //insert goto jumpDest after each org.processj.yield as we want the method to return.
                mn.instructions.insert(node, new JumpInsnNode(Opcodes.GOTO,
                        jumpDest));
            }
        }
    ...
    private void verifyModifiedClass(ClassWriter cw) {
        ...
        org.processj.compiler.Compiler.Info("==============================");
        org.processj.compiler.Compiler.Info("*  Verifying modified class  *");
        org.processj.compiler.Compiler.Info("==============================");
        ...
        org.processj.compiler.Compiler.Info("Result:" + sw.toString());
    }
    ...
    private int getOperand(String clazz, final AbstractInsnNode node) {
        ...
        switch (node.getType()) {
            case AbstractInsnNode.VAR_INSN:
                 ...
                 * org.processj.yield/resume. e.g. org.processj.yield(x)
                 *
                ...
                int var = vis.var;
                org.processj.compiler.Compiler.Info(" > vis opcode=" + opvis + " value=" + var);
                break;
    ...
    private void cleanup(MethodNode mn) {
        ...
        while (it.hasNext()) {
            ...
            if (n.getOpcode() == Opcodes.ATHROW)
                org.processj.compiler.Compiler.Info("athrow found");//we want to remove this
            else if (n.getOpcode() == Opcodes.NOP)
                org.processj.compiler.Compiler.Info("nop found");//we want to remove this
        }
    ...
    public static void main(String[] args) {
        try {
            ...
            if (args.length == 1) {
                ...
                org.processj.compiler.Compiler.Info("Print Usage!!!");
            }
